...
export default function User() {
  ...
  useEffect(() => {
    // Fetch from cache only - we rely on Login response for fresh data
    const cachedUser = localStorage.getItem("-user-data");
    if (cachedUser) {
      try {
        const d = JSON.parse(cachedUser);
        setUserName(d.full_name ?? d.username ?? "Pengguna");
        setUserEmail(d.email ?? "");
        const photo = d.teacher?.photo_url ?? d.student?.photo_url ?? "";
        setUserAvatar(photo);
      } catch (e) {
                  ...
                  <CardContent>
                    <Box className="max-w-64 sm:w-72 sm:max-w-none">
                      <Box className="mb-4 flex flex-col items-center">
                        <Avatar alt="avatar" src={userAvatar} className="large mb-2" />
                            ...
                            >
                              <Box className="flex w-full flex-row items-center justify-between">
                                ...
                                <AvatarGroup max={3} className="tiny transition-opacity group-aria-expanded:opacity-0">
                                  <Avatar className="tiny" alt="Bapak Guru" src="/images/avatars/avatar-3.jpg" />
                                  <Avatar className="tiny" alt="Ibu Guru" src="/images/avatars/avatar-2.jpg" />
                                  <Avatar className="tiny" alt="Staf Sekolah" src="/images/avatars/avatar-7.jpg" />
                                  <Avatar className="tiny" alt="Kepala Sekolah" src="/images/avatars/avatar-4.jpg" />
                                </AvatarGroup>
                              </Box>
                            </Button>
                          </AccordionSummary>
                      ...
                      <Button
                        ...
                        onClick={async (event) => {
                          ...
                          try {
                            ...
                            await fetch(`${DEFAULTS.API_URL}/api/v1/auth/logout`, {
                              method: "POST",
                              credentials: "include", // Include HttpOnly cookie for refresh_token
                              headers: token ? { Authorization: `Bearer ${token}` } : {},
                            });
                          } catch {
                            // Ignore network errors on logout
                          } finally {
                            // Clear only session and user profile data
