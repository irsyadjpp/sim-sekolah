...
class RawImageMetadata:
    ...
    # MinIO storage
    minio_key: Optional[str] = None  # MinIO object key (jika di-upload)
...
class ImageExtractionResult:
    ...
    file_path: str
    doc_id: Optional[str] = None    # Document ID untuk MinIO upload
    total_pages: int = 0
    total_images_found: int = 0
    total_images_extracted: int = 0
    total_skipped_small: int = 0
    total_skipped_noise: int = 0
    total_duplicates: int = 0
    total_uploaded_minio: int = 0
...
class ImageExtractor:
    ...
    def __init__(
        ...
        extract_bytes: bool = False,    # Default False karena upload ke MinIO
        extract_base64: bool = False,
        extract_surrounding_text: bool = True,
        skip_noise: bool = True,
        deduplicate: bool = True,
        upload_to_minio: bool = True,   # Default True untuk upload ke MinIO
    ):
        ...
        # Initialize MinIO client jika diperlukan
    ...
    def extract(
        ...
        Args:
            ...
            doc_id: Document ID untuk MinIO upload (jika upload_to_minio=True)
        ...
        try:
            ...
            for page_num in range(start_page, min(end_page, doc.page_count)):
                page = doc[page_num]
                page_images = self._extract_from_page(doc, page, page_num + 1, doc_id)
                ...
                for img_meta in page_images:
                    ...
                    if img_meta.minio_key:
                        result.total_uploaded_minio += 1
            ...
            logger.info(
                ...
                f"{result.total_duplicates} duplikat, "
                f"{result.total_uploaded_minio} upload ke MinIO)"
            )
    ...
    # ─────────────────────────────────────────────
    # STREAMING API untuk large PDFs
    # ─────────────────────────────────────────────
    ...
    def extract_streaming(
        ...
        doc_id: Optional[str] = None,
        page_range: Optional[Tuple[int, int]] = None,
        callback: Optional[Callable[[RawImageMetadata], None]] = None,
    ) -> ImageExtractionResult:
        """
        Extract images secara streaming untuk PDF besar.
        Callback akan dipanggil untuk setiap gambar yang berhasil diekstrak.
        
        Args:
            file_path: Path ke file PDF
            doc_id: Document ID untuk MinIO upload
            page_range: Opsional (start, end) 1-indexed, inclusive
            callback: Function yang dipanggil untuk setiap gambar
            
        Returns:
            ImageExtractionResult dengan summary
        """
        result = ImageExtractionResult(
            file_path=file_path,
            doc_id=doc_id,
        ...
        try:
            ...
            for page_num in range(start_page, min(end_page, doc.page_count)):
                ...
                for img_meta in page_images:
                    ...
                    # Callback immediately untuk streaming processing
                    if callback:
                        try:
                            callback(img_meta)
                        except Exception as e:
                            logger.warning(f"Callback error untuk image {img_meta.image_id}: {e}")

            doc.close()
            logger.info(
                f"Streaming ekstraksi selesai: {result.total_images_extracted} gambar "
                f"dari {result.total_pages} halaman"
            )
    ...
    def _extract_from_page(
        ...
        for img_info in page.get_images(full=True):
            ...
            try:
                ...
                # ── Upload ke MinIO jika diperlukan ─────────
                minio_key = None
                if self.upload_to_minio and self.minio_client and doc_id:
                    try:
                        minio_key = self.minio_client.upload_image(
                            doc_id=doc_id,
                            ...
                            metadata={
                                "width": str(img_w),
                                "height": str(img_h),
                                "colorspace": str(img_cs),
                                "image_type": image_type.value,
                            }
                        )
                        if minio_key:
                            logger.debug(f"Uploaded to MinIO: {minio_key}")
                    except Exception as e:
                        logger.warning(f"Error uploading to MinIO: {e}")
    ...
    def _map_colorspace(self, cs_code: int) -> ImageColorspace:
        """Map PyMuPDF colorspace codes ke ImageColorspace"""
        mapping = {
            0: ImageColorspace.GRAY,     # DeviceGray
            1: ImageColorspace.RGB,      # DeviceRGB
            2: ImageColorspace.CMYK,     # DeviceCMYK
            3: ImageColorspace.GRAY,     # CalGray
            4: ImageColorspace.RGB,      # CalRGB
            5: ImageColorspace.UNKNOWN,  # Lab
            6: ImageColorspace.UNKNOWN,  # ICC-based
        }
