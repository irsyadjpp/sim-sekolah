...
from common.schemas.common import HealthCheckResponse as HealthResponse, Response as BaseResponse, ErrorResponse
...
# Prometheus metrics (use custom registry to avoid duplicates)
...
active_connections = Gauge('gateway_active_connections', 'Active connections', registry=metric_registry)
...
# Service registry
SERVICES = {
    "parser": {"url": "http://localhost:8001", "timeout": 30.0},
    "semantic-chunk": {"url": "http://localhost:8003", "timeout": 30.0},
    "metadata": {"url": "http://localhost:8004", "timeout": 30.0},
    "embedding": {"url": "http://localhost:8005", "timeout": 30.0},
    "retrieval": {"url": "http://localhost:8006", "timeout": 30.0},
    "generation": {"url": "http://localhost:8007", "timeout": 60.0},
    "audit": {"url": "http://localhost:8008", "timeout": 30.0},
    "monitoring": {"url": "http://localhost:8012", "timeout": 30.0},
}

# Rate limiting (simple in-memory, use Redis for production)
rate_limit_store: Dict[str, list] = {}
# Use default rate limit if settings not available
RATE_LIMIT_REQUESTS = getattr(settings, 'rate_limit_per_minute', 60)
...
async def metrics():
    ...
    return FastAPIResponse(content=generate_latest(metric_registry), media_type="text/plain")
...
async def proxy_to_service(
    ...
    path: str,
    request: Request,
    auth_payload: Optional[Dict[str, Any]] = Depends(verify_jwt if not getattr(settings, 'debug', True) else None)
):
...
if __name__ == "__main__":
    uvicorn.run(
        ...
        reload=False  # Disabled to avoid duplicate metrics registration
    )